<template>
  <div>
    <div class="flex justify-between items-center mb-8">
      <h1 class="text-3xl font-bold">Гайды</h1>
      <NuxtLink to="/guides/create" class="bg-nord-3 text-nord-6 px-6 py-2 rounded-lg hover:bg-nord-8">
        Опубликовать гайд
      </NuxtLink>
    </div>
...
const uniqueTags = computed(() => {
  const tags = guides.value.flatMap(item => item.tags)
  return [...new Set(tags)]
})

const filteredGuides = computed(() => {
  if (!selectedTag.value) return guides.value
  return guides.value.filter(item => item.tags.includes(selectedTag.value))
})
</script>